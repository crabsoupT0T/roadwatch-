...
/// Reports within this many meters of an existing pothole add a vote to it
/// instead of creating a duplicate pin. Matches the 10m radius drawn on the map.
...
pub async fn report_pothole(pool: &PgPool, req: ReportRequest) -> Result<ReportResponse, AppError> {
    ...
    // PostGIS does the radius search with its spatial index (GIST), so this
    // stays fast even with a large number of reported potholes.
    ...
    let response = if let Some(existing) = nearby {
        // The UNIQUE(pothole_id, phone_hash) constraint on votes is what actually
        // enforces "one vote per phone number per pothole" — even under concurrent
        // requests, the database (not application logic) makes the call.
